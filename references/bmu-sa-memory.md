...
This file contains only facts confirmed by a user-provided, working SQL query. Expand it when additional schema details and business rules are verified.
...
## Rules confirmed by the grant-statistics query

- `g.IsConfirmed = 1` selects confirmed grants.
- `g.AcademicYearID = 6` limits the report to Academic Year 6.
- `s.StatusID IN (1, 12)` defines the included student statuses for this report.
- `spe.StatusID = 1` selects the active programme enrolment for this report.
- `spe.ProgramID <> 18` excludes Programme 18.
- `g.Percentage <> 'FIX'` excludes fixed-amount grants.
- A student with any confirmed, non-`FIX` grant in Academic Year 6 is excluded from the `No Grant` group.
- The result groups students by `YearOfStudy`, grant percentage/type, and `ProgramTypeName`.
- `COUNT(DISTINCT s.StudentID)` prevents a student from being counted more than once inside a result group.

## Important scope warning

Do not assume these numeric IDs have the same meaning in every report. Preserve them for this verified query, but ask for confirmation before reusing or changing them in a different business context.
